...
//      logger.warn("Fragment data: endpoint - {}, m{}:{}", fragment.getEndpoint(), fragment.getHandle().getMajorFragmentId(),fragment.getHandle().getMinorFragmentId());
//    }
    ...
    for (FragmentData fragment : fragments) {
      if (! (activeDrillbits.contains(fragment.getEndpoint())) ) {
        ...
        UserBitShared.DrillPBError error = ErrorHelper.logAndConvertError(fragment.getEndpoint(), "Failure while sending fragment.", new Throwable("Drillbit not RESPONDING"), logger);
        failWithError(error);
        break;
      }
    }
  ...
  private void finished(FragmentStatus status){
    ...
    logger.warn("finished fragment {}:{}. RemainingFragmentCount: {}", status.getHandle().getMajorFragmentId(), status.getHandle().getMinorFragmentId(), remaining);
...
//    this.status.setEndTime(System.currentTimeMillis());
  ...
  private void stopQuery(){
    logger.warn("Stop query. {}", queryId, status.toString());
  ...
  private class FragmentSubmitListener extends EndpointListener<Ack, InitializeFragments>{
    ...
    public void failed(RpcException ex) {
      ...
      //boolean verbose = cont.getOptions().getOption(ExecConstants.ENABLE_VERBOSE_ERRORS_KEY).bool_val;
