...
public class ProfileResources {
  ...
  public QProfiles getProfilesJSON() {
    ...
    try {
      ...
      eStore = work.getContext().getPersistentStoreProvider().getEStore(QueryStatus.QUERY_PROFILE);
    } catch (IOException e) {
    ...
    for (Map.Entry<String, QueryProfile> entry : pStore) {
      QueryProfile profile = entry.getValue();
      if (profile.getState() == QueryState.COMPLETED) {
  ...
  private QueryProfile getQueryProfile(String queryId) {
    ...
    if (profile != null) {
      assert (profile.getState() == QueryState.RUNNING || profile.getState() == QueryState.PENDING);

      if (profile.getForeman() == work.getContext().getEndpoint()) {
  ...
  public String cancelQuery(@PathParam("queryid") String queryId) throws IOException {
    EStore<QueryProfile> profiles = work.getContext().getPersistentStoreProvider().getEStore(QueryStatus.QUERY_PROFILE);
